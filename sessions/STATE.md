...
Первый этап плана завершён. Бизнес-логика ещё не начата.

## Не начато

- Контрактные тесты с ожидаемым первым падением.
...
- Task API пока возвращает `501` по всем пяти маршрутам: это намеренная заглушка
  первого этапа, а не готовая реализация.

## Следующий шаг

Начать новую сессию и выполнить второй этап `IMPLEMENTATION_PLAN.md`: записать
контрактные feature-тесты и показать их ожидаемое падение на `501` до реализации.
